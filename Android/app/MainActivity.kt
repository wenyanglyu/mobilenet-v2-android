...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadLabels() {
        try {
            val inputStream = assets.open("imagenet_labels.json")
            val jsonString = inputStream.bufferedReader().use { it.readText() } // Read full JSON
            val jsonObject = JSONObject(jsonString) // Parse JSON as JSONObject

            // Convert JSON keys (strings) to Int and create a map
            labelsMap = jsonObject.keys().asSequence()
                .associate { key -> key.toInt() to jsonObject.getString(key) }
    ...
    private fun getLabelForIndex(index: Int): String {
        val label = labelsMap[index]
        if (label == null) {
            Log.e("TFLite", "debug1001 - Invalid Index: $index, Label Not Found!")
            return "Unknown (Invalid Index: $index)"
        }
        return label
    }
